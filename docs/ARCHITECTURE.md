...
Codex CLI 可能将 `Reconnecting... N/M (...)` 重连通知也标为 `error`。只有通知之后出现 `turn.completed`、进程成功退出且结构化结果有效时，才视为已恢复，并在 `diagnostics.json` 的 `warnings` 中保留通知。其他错误与任何终止失败事件仍然拒绝；完成之后出现的错误不能被先前的完成事件掩盖。
